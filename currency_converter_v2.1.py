...
class Converter:
    def __init__(self):
        ...
        self.from_usd_button = Radiobutton(self.from_buttons_frame,
                                           text="USD $",
                                           padx=0, pady=0,
                                           indicatoron=0,
                                           value=0,
        ...
        self.from_nzd_button = Radiobutton(self.from_buttons_frame,
                                           text="NZD $",
                                           padx=0, pady=0,
                                           bg=background,
                                           fg=txt_colour,
        ...
        self.from_rmb_button = Radiobutton(self.from_buttons_frame,
                                           text="RMB ¥",
                                           padx=0, pady=0,
                                           bg=background,
                                           fg=txt_colour,
        ...
        # binds left button press to the entry box to clear texts on click
        self.to_convert_entry.bind("<Button-1>", self.value_reset)
        ...
        # 'Convert from' buttons
        self.to_usd_button = Radiobutton(self.to_buttons_frame,
                                         text="USD $",
                                         bg=background,
                                         fg=txt_colour,
                                         indicatoron=0,
        ...
        self.to_nzd_button = Radiobutton(self.to_buttons_frame,
                                         text="NZD $",
                                         bg=background,
                                         fg=txt_colour,
                                         indicatoron=0,
        ...
        self.to_rmb_button = Radiobutton(self.to_buttons_frame,
                                         text="RMB ¥",
                                         bg=background,
                                         fg=txt_colour,
                                         indicatoron=0,
    ...
    def value_reset(self, event):  # including the event of clicking this entry box as an argument
        # clears all the texts in the entry box
        self.to_convert_entry.delete(0, "end")
        return None

    # function to add chosen currency to convert from to a list
    def convert_from(self, curr):
        # deletes default text in the hidden from_curr entry box
        self.from_curr.delete(0, END)
        # inserts newly selected currency in the hidden entry box
        self.from_curr.insert(0, curr)
        # sets the selected currency to the 1st item in chosen_currencies list
        chosen = self.from_curr.get()
        chosen_currencies[0] = chosen

    # function to insert chosen currency to convert to into the chosen_currencies list
    def convert_to(self, curr):
        # deletes default text in the hidden to_curr entry box
        self.to_curr.delete(0, END)
        # inserts newly selected currency in the hidden entry box
        self.to_curr.insert(0, curr)
        # sets the selected currency to the 2st item in chosen_currencies list
        chosen = self.to_curr.get()
        chosen_currencies[1] = chosen
        ...
        try:
            ...
            if from_ == "USD" and value > 0:
                ...
                if to_ == "USD":
                    ...
                    error_msg = "Choose a currency to convert to"
                ...
                if to_ == "USD":
                    ...
                    error_msg = "Choose a different currency"
                else:
                    error_msg = "Choose a currency to convert to"
            elif from_ == "from":
                error_msg = "Choose a currency to convert from"
...
chosen_currencies = ["from", "to"]

# list for storing calculation history
all_conversions = []

# instruction for the help GUI
instruction = "Converting a price from one currency to another: \n" \
              "*Press the name of the currency that the price is currently in \n" \
              "*Enter the price at the entry box \n" \
              "*Press the name of the currency that you wish to convert to \n" \
              "*Press the blue ‘Convert’ button \n" \
              "*The converted value will appear in a box to the right of the ‘Convert’ button \n" \
              "\r\n" \
              "Conversions will not run if: \n" \
              "*The currency that the value is in is the same as the currency you wish to convert to \n" \
              "*The price you entered is not a number \n" \
              "*You only pressed either the currency that the price is in or the currency you want to convert to \n" \
              "*You did not press the ‘Convert’ button after choosing the currencies and entering a value \n" \
 \
    # Main routine
